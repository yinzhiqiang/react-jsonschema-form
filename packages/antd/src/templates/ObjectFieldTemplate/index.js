...
const ObjectFieldTemplate = ({
  ...
  let { colSpan = 24, labelAlign = "right", rowGutter = 24 } = formContext;

  if (uiSchema["ui:rowGutter"]) {
    rowGutter = uiSchema["ui:rowGutter"];
  }
  ...
  const findUiSchemaColSpan = (element) => findUiSchema(element)["ui:colSpan"];

  const calculateColSpan = (element) => {
    const elementColSpan = findUiSchemaColSpan(element);
    if (elementColSpan) {
      return elementColSpan;
    }
